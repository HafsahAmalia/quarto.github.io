...
```{r , echo=FALSE, out.width = '100%', fig.cap='**Ref**: Altman. Systematic reviews of evaluations of prognostic variables. BMJ 2001'}
...
- $\hat{\theta}_k$ represents the estimated factor-outcome association
  (e.g. log hazard ratio) of study *k*, 
- $V(\hat{\delta_k})$ is the error variance 
- **$D_F$ is the fixed effect meta-analysis summary** 
  (e.g. pooled log hazard ratio)
...
- **$D_R$ is the random effect meta-analysis summary** 
  (e.g. pooled log hazard ratio)
...
- Focusing on the mean is insufficient, and the effect of heterogeneity should be quantified
